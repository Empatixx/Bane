...
public class Game implements Runnable {
    ...
    private void initWindow() {
        ...
        // Create the window
        window = glfwCreateWindow(WIDTH, HEIGHT, "2D Game", glfwGetPrimaryMonitor(), NULL);
        if ( window == NULL )
        ...
        try {
            ...
            ByteBuffer buffer = cursorDecoder.decodeImage("Textures\\cursor.tga");

            cursorImage.set(cursorDecoder.getWidth(), cursorDecoder.getHeight(), buffer);

            //long cursor = GLFW.glfwCreateCursor(cursorImage, 32, 32);
            long cursor = GLFW.glfwCreateCursor(cursorImage, 12, 7);
