...
public class TileMap {
	...
	public TileMap(int tileSize, InGame gameState, Camera camera) {
		...
		// 2x scale
		numRowsToDraw = Game.HEIGHT / (tileSize*2) + 2;
		numColsToDraw = Game.WIDTH / (tileSize*2) + 2;
		...
		for (Room room : roomArrayList){
			...
			if (playerStartX > xMin && playerStartX < xMax){
				if (playerStartY > yMin && playerStartY < yMax){
					...
					xmin += Game.WIDTH - xMax - xmin;
					xmax += -xMin - xmax;

					ymin += Game.HEIGHT - yMax - ymin;
					ymax += -yMin - ymax;

					setPosition(Game.WIDTH / 2f - playerStartX,Game.HEIGHT / 2f - playerStartY);
					break;
				}
			}
		}
