...
public class MenuBar {
     ...
     */
    public MenuBar(String file, Vector3f pos, float scale, Camera camera, int width, int height){

        // resolution scaling
        float scaleX = Settings.scaleWidth();
        float scaleY = Settings.scaleHeight();
        pos.x *=scaleX;
        pos.y *=scaleY;
        width *=scaleX;
        height *=scaleY;
        ...
        buffer = BufferUtils.createDoubleBuffer(texCoords2.length);
        buffer.put(texCoords2);
        buffer.flip();
        vboTextures[1] = glGenBuffers();

        glBindBuffer(GL_ARRAY_BUFFER,vboTextures[1]);
        glBufferData(GL_ARRAY_BUFFER,buffer,GL_STATIC_DRAW);
        glBindBuffer(GL_ARRAY_BUFFER,0);
