...
public class TileMap {
	...
	public void loadTiles(String s) {
		try {
			...
			BufferedImage tileset2 = ImageIO.read(
				getClass().getResourceAsStream(s)
			);

			ByteBuffer tileset = ByteBufferImage.decodeImage(s);

			int height = tileset2.getHeight();
			int width = tileset2.getWidth();

			numTilesAcross = width / tileSize;
			tiles = new Tile[2][numTilesAcross];
			
			for(int col = 0; col < numTilesAcross; col++) {

				ByteBuffer subImage = tileset.duplicate();

				// GETTING RAW PIXELS OF SUBIMAGE
				glTexSubImage2D(GL_TEXTURE_2D,0,col*tileSize,0,width,height,GL_RGBA,GL_UNSIGNED_BYTE,subImage);

				tiles[0][col] = new Tile(subImage, Tile.NORMAL,tileSize);

				ByteBuffer subImage2 = tileset.duplicate();

				glTexSubImage2D(GL_TEXTURE_2D,0,col*tileSize,tileSize,width,height,GL_RGBA,GL_UNSIGNED_BYTE,subImage2);

				tiles[1][col] = new Tile(subImage2, Tile.BLOCKED,tileSize);
			}
		}
		catch(Exception e) {
			e.printStackTrace();
