		...
		for(int loop = 0;loop < 3;loop++) {
			...
			// finding maxCols and maxRows of final tile map
						...
						if (mistnost == null) {
							if (y != roomY -1){
								for (int i = 0; i < roomY; i++){
									Room bottomRoom = getRoom(roomMap[i][x]);
									if (bottomRoom != null){
										int cols = bottomRoom.getNumCols();
										int rows = bottomRoom.getNumRows();
								...
								for (int i = 1; i < roomY; i++){
									Room bottomRoom = getRoom(roomMap[y-i][x]);
									if (bottomRoom != null){
										int cols = bottomRoom.getNumCols();
										int rows = bottomRoom.getNumRows();
				...
				for (int j = 0; j < roomY; j++) {
					...
					for (int k = 0; k < roomX; k++) {
						Room mistnost = getRoom(roomMap[j][k]);
						// POKUD ZDE ZADNA MISTNOST NENI
						if (mistnost == null){

							// TODO: predelat nebude fungovat pri custom cols/rows
							// pokud mistnost neni v poslednim radku - loopujeme dolu
							if (j != roomY - 1){
								for (int i = 0; i < roomY; i++){
									Room bottomRoom = getRoom(roomMap[i][k]);
									if (bottomRoom != null){
										int cols = bottomRoom.getNumCols();
										int rows = bottomRoom.getNumRows();
							...
							// pokud mistnost je dole loopuje od zdola nahoru
							} else {
								for (int i = 1; i < roomY; i++){
									Room bottomRoom = getRoom(roomMap[j-i][k]);
									if (bottomRoom != null){
										int cols = bottomRoom.getNumCols();
										int rows = bottomRoom.getNumRows();
										if (nextShiftRows < rows){
							...
							for(int x = 0;x < rows;x++){
								...
								for(int y = 0;y < cols;y++){
									...
									map[x+shiftRows][y+shiftCols] = roomMap[x][y];
								}
							}
							...
							if (mistnost.getId() == 1){
								...
								playerStartX = xMin+(xMax-xMin)/2;
								playerStartY = yMin+(yMax-yMin)/2;
